...
- (BOOL)initConfig
{
    ...
    [MobClick startWithAppkey:UmengAppkey reportPolicy:BATCH   channelId:nil]; //default appstore
    ...
    [UMSocialQQHandler setQQWithAppId:QQ_APPID appKey:QQ_APPSECKET url:SHAREURLROOT];
    [UMSocialWechatHandler setWXAppId:WCHAT_APPID appSecret:WCHAT_APPSECKET url:SHAREURLROOT];
    [UMSocialSinaSSOHandler openNewSinaSSOWithRedirectURL:SINA_REDIRECTURL];
    
    
    [SMSSDK registerApp:SMS_APPID withSecret:SMS_APPSCECRET];
...
- (BOOL)shareListVC:(UIViewController *)controller loginType:(HCLoginType)loginType url:(NSString *)url shareTitle:(NSString *)title shareContent:(NSString *)content shareImg:(id)image  imgUrlString:(NSString *)imageUrlString completed:(ShareCompleted)success
{
    ...
    {
        url = [NSString stringWithFormat:SHAREURL,url,(int)loginType,(long)0,(long)0];
    }
    ...
    CLLocationCoordinate2D loc = CLLocationCoordinate2DMake(config.AccuraceLat,config.AccuraceLng);
    
    CLLocation * location = nil;
    if(CLLocationCoordinate2DIsValid(loc))
    {
        location =  [[CLLocation alloc]initWithLatitude:config.AccuraceLat longitude:config.AccuraceLng];
    }
    
    if(loginType == HCLoginTypeWeixin)
    {
        //设置微信AppId，设置分享url，默认使用友盟的网址
        [UMSocialWechatHandler setWXAppId:WCHAT_APPID appSecret:WCHAT_APPSECKET url:url];
        {
    ...
    else if(loginType == HCLoginTypeQQ)
    {
        ...
        [UMSocialQQHandler setQQWithAppId:QQ_APPID appKey:QQ_APPSECKET url:url];
